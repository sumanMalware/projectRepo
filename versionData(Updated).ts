...
let androidVersion: androidData[] = [
    ...
    },
    {
        ...
        date: "09-02-2009",
        bugs: "864785",
        features: ["Details and reviews available when a user searches for businesses on Maps.", "Longer in-call screen timeout by default when using the speakerphone, plus the ability to show/hide the dialpad.", "Ability to save attachments in messages.", "Support added for marquee in system layouts."],
        author: ["rahul patel"],
    ...
    },
    {
        ...
        date: '15-09-2009',
        bugs: "864835",
        features: ["Voice and text entry search enhanced to include bookmark history, contacts, and the web.", "Ability for developers to include their content in search results.", "Ability to save attachments in messages."
    ...
    },
    {
        ...
        bugs: "865231",
        bugFixed: "yes",
        features: ["Expanded Account sync, allowing users to add multiple accounts to a device for synchronization of an email and contacts.", "Microsoft Exchange email support, with a combined inbox to browse an email from multiple accounts in one page."
...
function findByReleaseYear(year: string) {
    ...
    let filteredYear = androidVersion.filter(n => n.date.toString().includes(year))
...
findByFeatures("Ability for developers to include their content in search results.")
...
const sortingForObject = (object1: any) => {                //sorting the object of Author
    let object2: any = {};
    let array1: any = [];
...
for (let i in sortingForObject(counts)) {
    console.log(`3. who is the Author who worked on maximum numbers releasd?`)
     console.log(`${i} is the Author who is contrubuting most in releases, with ${sortingForObject(counts)[i]} times`)
    break
}
}
